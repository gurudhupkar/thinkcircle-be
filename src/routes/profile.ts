    ...
    try {
      ...
      if (profiles.length === 0) {
        return res.status(404).json({
          message: "No profiles found for the given subject",
          success: false,
        });
      }
...
//         return res.status(400).json({
//           message:
//             "One or both users are already in a group. Cannot form a new group.",
...
//         return res.status(400).json({
//           message:
//             "Cannot form group with a user having completely different subjects",
